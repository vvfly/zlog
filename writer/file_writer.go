...
func (f *fileWriter) Check(entry *ZEntry) bool {
	if f.opt.CheckFunc != nil {
		return f.opt.CheckFunc(entry)
	}

	return checkLogLevel(entry.Loglevel, f.opt.LogLevel)
}

func (f *fileWriter) Write(message string, attr map[string]interface{}) error {

	_, err := f.log.Write([]byte(message))
	return err
}
